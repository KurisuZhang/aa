...
export function MyNavbar({ setProducts }) {
  ...
  const handleCategoryClick = (category) => {
    ...
    axios
      ...
      .catch((error) => {
        console.error(
          `There was an error fetching ${category} products!`,
          error,
        );
      });
  };
  ...
  const handleSearchClick = () => {
    ...
    axios
      .get(url)
      .then((response) => {
        setProducts([response.data]); // Assuming the API returns a single product
      })
      .catch((error) => {
        console.error(
          `There was an error fetching the product with ID ${searchValue}!`,
          error,
        );
      });
  };
  ...
  return (
    <Navbar className="mx-auto w-9/12 px-4 py-2 lg:px-8 lg:py-4">
